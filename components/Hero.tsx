...
// Dynamically import Calendly to avoid SSR issues and preload it
const CalendlyWidget = dynamic(
  () => import('react-calendly').then((mod) => mod.InlineWidget),
  { ssr: false }
);
...
const Hero = () => {
  ...
  useEffect(() => {
    setIsLoaded(true);
    // Preload Calendly on mount
    import('react-calendly').then(() => {});
  }, []);
    ...
    <section className="relative overflow-hidden bg-gradient-to-b from-black via-blue-950 to-black min-h-[90vh] flex items-center">
      <div className="container mx-auto px-4 py-20 relative z-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          >
            <motion.h1 
              className="text-4xl md:text-5xl lg:text-6xl font-bold mb-6 bg-clip-text text-transparent bg-gradient-to-r from-white via-blue-200 to-blue-400"
            >
              Transform Your Business with Expert Digital Solutions
            </motion.h1>
            ...
            <motion.div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start">
              <Button onClick={() => setShowCalendly(true)} className="relative group px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-lg font-medium text-lg transition-all duration-300 flex items-center justify-center gap-2 w-full sm:w-auto">
                <Calendar className="w-5 h-5" />
                <span>Book Free Consultation</span>
                <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                <span className="absolute -top-2 -right-2 bg-red-500 text-white text-xs px-2 py-0.5 rounded-full">Free</span>
              </Button>
            </motion.div>
          </motion.div>
        </div>
      ...
      {/* Right Column - Staggered Images with Microinteractions */}
      <motion.div 
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: isLoaded ? 1 : 0, scale: isLoaded ? 1 : 0.95 }}
        transition={{ duration: 0.7, delay: 0.3, ease: "easeOut" }}
        ...
        <div className="relative flex space-x-4">
          {[1, 2, 3].map((i) => (
            <motion.div
              ...
              className="rounded-xl overflow-hidden shadow-lg border border-blue-500/20 bg-blue-900/10 backdrop-blur-sm p-1"
            >
              <Image
                src={`https://source.unsplash.com/400x300/?technology,business&random=${i}`}
      ...
      {/* Calendly Modal */}
      {showCalendly && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50" aria-modal="true" role="dialog" aria-label="Calendly scheduling modal">
          <div className="bg-white p-6 rounded-lg shadow-lg max-w-md relative">
            ...
            <CalendlyWidget url="https://calendly.com/your-schedule" />
          </div>
        </div>
      )}
    </section>
